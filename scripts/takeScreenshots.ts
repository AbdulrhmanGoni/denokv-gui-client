...
async function takeScreenshotOfTheme(page: Page, name: string) {
  await page.screenshot({
    path: `./screenshots/${name}_light.temp.png`,
    fullPage: true,
  });
  await page.keyboard.press("Alt+s"); // toggle to dark mode
  await page.screenshot({
    path: `./screenshots/${name}_dark.temp.png`,
    fullPage: true,
  });
...
async function takeScreenshotOfKvEntriesTable(page: Page) {
  await page
    .locator("#kv-stores-grid > div", { hasText: "Some Local KV Store" })
...
async function takeScreenshotOfEnqueueMessageDialog(page: Page) {
  ...
  await page
    .locator("div", {
      has: page.locator("> p", { hasText: "Backoff Schedule" }),
    })
    .locator("button")
    .click();
  await page.waitForTimeout(100);
  ...
  await page
    .locator("div", {
      has: page.locator("> p", { hasText: "Keys If Undelivered" }),
    })
    .locator("button")
    .click();
  await page.waitForTimeout(100);
...
async function takeScreenshotOfAtomicOperationsDialog(page: Page) {
  ...
  await enqueueValueEditor.fill(
    '{task: "register-fide-rating", value: {user: "abdulrhman", rating: 2525}}',
  );
  ...
  const watchedEntries = [
    {
      ...
      value: {
        type: "Object",
        data: '{"name": "abdulrhman", "title": "IM", "rating": 2525}',
      },
    },
    {
      key: ["A", "watched", "Set"],
