...
export function atomicOperationsTests() {
  ...
  test("Check that a Kv Entry already exists to avoid re-setting it again", async ({
    page,
  }) => {
    await page.locator("button", { hasText: "check" }).click();
    await page
  ...
  test("All operations fail due to one failed operation in the transaction", async ({
    page,
  }) => {
    await page.evaluate(async () => {
      const kvClient = globalThis["kvClient" as keyof typeof globalThis];
  ...
  test("Use 'sum', 'min', and 'max' operations in one atomic transaction", async ({
    page,
  }) => {
    await page.locator("button", { hasText: "Reset" }).click();
