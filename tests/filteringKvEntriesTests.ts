...
export function filteringKvEntriesTests() {
  ...
  test("Check all entries are fetched", async ({ page }) => {
    ...
    await expect(page.locator("tr")).toHaveCount(
      randomTestingKvEntries.length + usersTestingKvEntries.length + 1,
    );
  });
  ...
  test("Apply 'start' & 'end' range filter and check only entries in range are fetched", async ({
    ...
    await page
      .locator("button", {
        hasText: "Reset",
        has: page.locator("svg.lucide-funnel-x"),
      })
      .click();
  ...
  test("Apply a random filter and check that no entries are fetched", async ({
    ...
    await page.locator("button", { hasText: "Apply" }).click();
    await expect(
      page.locator("tr", { hasText: "No Entries in this Deno Kv Store." }),
    ).toHaveCount(1);
  });
  ...
  test("Update the saved filter to return entries in reverse order, re-apply it and check that it is applied", async ({
    ...
    const dialog = "div[data-slot='dialog-content'][data-state='open']";
    await page
      .locator(`${dialog} button[data-slot="tooltip-trigger"]`, {
    ...
    for (let i = usersTestingKvEntries.length; i >= 1; i--) {
      await expect(usersRows.nth(i - usersTestingKvEntries.length)).toHaveText(
        new RegExp(`[  "users" ,${i}  ]`),
      );
    }
  });
  ...
  test("Delete the saved filter", async ({ page }) => {
    ...
    const dialog = "div[data-slot='dialog-content'][data-state='open']";
    await page
      .locator(`${dialog} button[data-slot="alert-dialog-trigger"]`, {
    ...
    await expect(page.locator(`${dialog} p`).last()).toHaveText(
  ...
  test("Apply a small page-size via 'limit' option and navigate through multiple pages of entries", async ({
    page,
  }) => {
    await page
      .locator("button", {
