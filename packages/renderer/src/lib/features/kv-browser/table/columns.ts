import { renderComponent, renderSnippet } from "$lib/ui/shadcn/data-table";
...
export const columns: ColumnDef<KvEntry>[] = [
  ...
  },
  {
    ...
    header: "Key",
    cell: ({ row }) =>
      renderComponent(KvKeyRenderer, {
        key: row.original.key,
        className: "max-w-40 md:max-w-46 xl:max-w-52 2xl:max-w-60",
      }),
  },
  {
    accessorKey: "value",
    header: "Value",
    cell: ({ row }) =>
      renderComponent(KvValueRenderer, {
        value: row.original.value,
        className: "max-w-80 md:max-w-[500px] xl:max-w-2xl",
      }),
  },
  {
    ...
    cell: ({ row }) =>
      renderSnippet(
        createRawSnippet(() => ({
          render: () =>
            `<div class="text-right line-clamp-1 max-w-[175px] overflow-auto py-1">${row.getValue("versionstamp")}</div>`,
        })),
        "",
      ),
  },
