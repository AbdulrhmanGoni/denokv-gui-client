<script lang="ts">
  import { settingsPageState, setHardwareAccelerationMode, settingsState } from "$lib/states/settingsState.svelte";
  ...
  let hardwareAccelerationModeChanged = $derived(
    hardwareAccelerationEnabled === settingsPageState.isHardwareAccelerationCurrentlyDisabled
  )
</script>
...
<div class="space-y-2">
  ...
  <p class="text-muted-foreground text-sm">
    When enabled, the app will use your computer's graphics hardware (if available) to improve visual performance for smoother experience
  </p>
  <div class="flex items-center gap-1.5 bg-card p-2 rounded-md">
    ...
    />
    <span class="font-semibold">{hardwareAccelerationEnabled ? "Enabled": "Disabled"}</span>
    {#if hardwareAccelerationModeChanged}
      <Alert 
        ...
        Icon={OctagonAlert}
      />
      <button type="button" onclick={restartApp} class="text-sm flex items-center gap-1 cursor-pointer">
